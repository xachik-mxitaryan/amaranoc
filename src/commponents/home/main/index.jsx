...
export default function Home({ inputValue, setInputValue }) {
  ...
  useEffect(() => localStorage.setItem("homeFilters", JSON.stringify(filters)), [filters]);
  useEffect(() => localStorage.setItem("cardView", cardView), [cardView]);
  useEffect(() => localStorage.setItem("inputValue", inputValue || ""), [inputValue]);
  useEffect(() => localStorage.setItem("homePage", currentPage), [currentPage]);
  ...
  useEffect(() => {
    ...
    if (filters.peopleNight) data = data.filter((h) => Number(h.peopleSleepCaunt || 0) >= filters.peopleNight);

    if (filters.pool) data = data.filter((h) => (h.baseyn || "").includes(filters.pool));
    if (filters.sleep !== null) data = data.filter((h) => Boolean(h.isSleep) === Boolean(filters.sleep));
    if (filters.advantages?.length) {
      data = data.filter((h) => filters.advantages.every((a) => (h.advantages || []).includes(a)));
    }
        ...
        <div className="flex-1">
            ...
            <div className="flex items-center gap-6 overflow-x-auto no-scrollbar px-2 py-3">
              {main.map((item) => (
                ...
                >
                  ...
                  <span className={`text-xs ${active === item.id ? "text-black" : "text-gray-500"}`}>
                    {item.title}
                  </span>
                </button>
              ))}
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <div className="flex gap-3 mt-2">
              <button onClick={() => setCardView(2)} className={`p-2 rounded-lg ${cardView === 2 ? "bg-black text-white" : "bg-gray-200 hover:bg-gray-300"}`}>
                <CiGrid2V size={22} />
              </button>
              <button onClick={() => setCardView(3)} className={`p-2 rounded-lg ${cardView === 3 ? "bg-black text-white" : "bg-gray-200 hover:bg-gray-300"}`}>
                <BsGrid3X2GapFill size={22} />
              </button>
            </div>
          </div>

          <div className={`grid gap-6 ${cardView === 2 ? "grid-cols-1 sm:grid-cols-2" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"}`}>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center items-center gap-3 mt-10">
              <button onClick={() => setCurrentPage((p) => Math.max(p - 1, 1))} disabled={currentPage === 1} className="px-3 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 disabled:opacity-50">←</button>
              {[...Array(totalPages)].map((_, i) => (
                <button key={i} onClick={() => setCurrentPage(i + 1)} className={`px-4 py-2 rounded-full ${currentPage === i + 1 ? "bg-[#f97316] text-white font-semibold" : "bg-gray-100 hover:bg-gray-200"}`}>{i + 1}</button>
              ))}
              <button onClick={() => setCurrentPage((p) => Math.min(p + 1, totalPages))} disabled={currentPage === totalPages} className="px-3 py-2 rounded-lg bg-gray-200 hover:bg-gray-300 disabled:opacity-50">→</button>
            </div>
          )}
        </div>
      </div>
